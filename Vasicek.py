...
def Paths_V(NoOfPaths, NoOfSteps, T, P0t, mu, sigma, gamma, epsilon):
    """
    ...
    for iTime in range(1, NoOfSteps+1): # For each time increment
        ...
        rate_term = (1-np.exp(-gamma*dt))*mu
...
def mainCalculation_V(NoOfPaths, NoOfSteps, T, mu, sigma, gamma, P0t, epsilon):
    """
    ...
    paths = Paths_V(NoOfPaths, NoOfSteps, T, P0t, mu, sigma, gamma, epsilon)
    M = paths["M"]
    t = paths["time"]
    I = paths["I"]
...
def Run_Vasicek(asset_id):
    ...
    # Final comparison
    [t, P, implied_term_structure, M, I] = mainCalculation_V(NoOfPaths, NoOfSteps, T, mu, sigma, gamma, P0t, epsilon)
    ...
    out = pd.DataFrame(data = outTmp,columns=t,index=multi_index)
